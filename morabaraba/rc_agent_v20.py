...
class AI(MorabarabaPlayer):
    ...
    def play(self, state, remain_time):
        ...
        """
        print("<<<<<<<<<<<<", self.which_actions_r_guessed_2_b_the_best(
            state, self.position), ">>>>>>>>>>>")
    ...
    def which_actions_r_guessed_2_b_the_best(self, state, player):
        if(0):
            ...
            if(state.mill):
                    ...
                    while(max_score):
                        if(opponent_possibilities['action'][0].get_action() == "ADD"):
                            for index in np.linspace(0, opponent_mills_list.size-1, opponent_mills_list.size, dtype=np.int8)[opponent_possibilities['score'] == max_score]:
                                action_to_block = opponent_possibilities['action'][index].get_action_as_dict()[
                                    'action']
                                for mill in opponent_mills_list[index]:
                                    for cell in mill:
                                        if cell != action_to_block['to']:
                                            act = MorabarabaAction(
                                                action_type=MorabarabaActionType.STEAL, at=cell)
                                            if act not in res:
                                                res.append(MorabarabaAction(
                                                    action_type=MorabarabaActionType.STEAL, at=cell))
                        else:
                ...
                my_possibilities['score_making_mill'] = np.array(
                    my_possibilities['score_making_mill'])
        ...
        if(len(state.get_board().get_player_pieces_on_board(Color(player))) == 0):
            ...
            if(state.mill):
                ...
                if(len(opponent_possibilities['action']) != 0):
                    ...
                    index_maxScore = opponent_possibilities['score'].index(
                        maxScore)
                    if(maxScore):
                        actionToBlock = opponent_possibilities['action'][index_maxScore].get_action_as_dict()[
                            'action']
                        if(opponent_possibilities['action'][0].get_action() == "ADD"):
                            ...
                            act = MorabarabaAction(
                                action_type=MorabarabaActionType.STEAL, at=actionToBlock['at'])
                            if MorabarabaRules.is_legal_move(state, act, player):
                                print(
                                    ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 11")
                                return maxScore, act
                ...
                if(len(my_possibilities['action']) != 0):
                    for possibility in my_possibilities['action']:
                        state_copy = deepcopy(state)
                        MorabarabaRules.make_move(
                            state_copy, possibility, player)
                        ...
                        depth2_possibilities = MorabarabaRules.get_player_actions(
                            state_copy, player)
                        for depth2_possibility in depth2_possibilities:
                            if depth2_possibility.get_action_as_dict()['action']['to'] == cell:
                                can_mill, mills = self.is_a_mill_move(
                                    state_copy, player, depth2_possibility)
                                if(can_mill):
                                    print(
                                        ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 12")
                ...
                if(len(opponent_possibilities['action']) != 0):
                    ...
                    maxScore = max(opponent_possibilities['score'])
                    if(maxScore):
                        actionToBlock = opponent_possibilities['action'][opponent_possibilities['score'].index(
                            maxScore)].get_action_as_dict()['action']
                        if(len(my_possibilities['action']) != 0):
                            if(my_possibilities['action'][0].get_action() == "ADD"):
                                act = MorabarabaAction(
                                    action_type=MorabarabaActionType.ADD, to=actionToBlock['to'])
                                if MorabarabaRules.is_legal_move(state, act, player):
                                    print(
                                        ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 2")
                                    return maxScore, act
                    ...
                    if(my_possibilities["action"][0].get_action() == "ADD"):
                        ...
                        for i in range(n+1):
                            ...
                            if cell in opponent_pieces:
                                cpt -= 1
                            elif cell in player_pieces:
                                cpt += 1
                        if cpt > 0 and AI.next_square_vertex(self.vip_square, board_shape) not in opponent_pieces and AI.next_square_vertex(self.vip_square, board_shape) not in player_pieces:
                            self.vip_square = AI.next_square_head(
                                self.vip_square, board_shape)
                            act = MorabarabaAction(
                                action_type=MorabarabaActionType.ADD, to=self.vip_square)
                            if MorabarabaRules.is_legal_move(state, act, player):
                                print(
                                    ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 3")
                                return 0, act
                            ...
                            print(self.vip_square)
                            vip_diagonal = AI.which_lines_pass_through(
                                AI.next_square_head(self.vip_square, board_shape), board)["diagonal"]
                            for diaginal_hole in vip_diagonal:
                                cpt = 0
                                cell = diaginal_hole
                                for i in range(n+1):
