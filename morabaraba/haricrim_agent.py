...
class AI(MorabarabaPlayer):
    ...
    def play(self, state, remain_time):
        ...
        my_milles_on_board=board.player_mills()
    ...
    def strategie(self,state):
        ...
        my_pieces_on_board=state.get_board().get_player_pieces_on_board(Color(self.position))
        adv_pieces_on_board=state.get_board().get_player_pieces_on_board(Color(-self.position))
        in_my_hand=state.get_player_info(Color(self.position))['in_hand'] #maybe une erreur de synthaxe ici
        if in_my_hand==0:
            return 'attack'
        else:
            return 'defense'
    ...
    def possibilities2(self,state):
        ...
        for move  in MorabarabaRules.get_player_actions(state, self.position):
            ...
            new_state = MorabarabaRules.act(temp_state, move)[0]
            possibilities2.append([new_state, move])
    ...
    def possibilities(self,state):
        ...
        possibilities=[]
        if len(state.get_player_info(Color(self.position))['in_hand'])==0:
            for piece in state.get_board().get_player_pieces_on_board(Color(self.position)):
                for move in state.get_board().get_effective_cell_moves(piece):
                    temp_state = deepcopy(state)
                    new_state = MorabarabaRules.act(temp_state, MorabarabaAction(action_type=MorabarabaActionType.MOVE, at=piece, to=move), self.position)[0]
                    possibilities.append([new_state,piece])
        
        elif len(state.get_player_info(Color(self.position))['in_hand'])>0:
            for piece in state.get_board().get_all_empty_cells():
                temp_state = deepcopy(state)
                new_state = MorabarabaRules.act(temp_state, MorabarabaAction(action_type=MorabarabaActionType.ADD, to=piece), self.position)[0]
                possibilities.append([new_state,piece])
            ...
            for piece in state.get_board().get_player_pieces_on_board(Color(self.position)):
                for move in state.get_board().get_all_empty_cells():
                    ...
                    new_state = MorabarabaRules.act(temp_state, MorabarabaAction(action_type=MorabarabaActionType.FLY, at=temp_piece, to=move), self.position)[0]
                    possibilities.append([new_state, temp_piece])
    ...
    def minimax(self, state, maximizingPlayer, depth, alpha=np.inf,  beta=-np.inf):
        ...
        if depth == 0 or MorabarabaRules.is_end_game(state):
            return self.evaluate(state, self.evaluate(state, self.strategie(state)))
