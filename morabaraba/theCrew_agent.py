...
class AI(MorabarabaPlayer):
    ...
    def play(self, state, remain_time):
        ...
        """
        print('--------------------------------')
        print('le mien',self.which_actions_r_guessed_2_b_the_best(state, self.position))
        print('le premier mien',self.which_actions_r_guessed_2_b_the_best(state, self.position)[0])
        print('--------------------------------')
        # print('le leur', MorabarabaRules.random_play(state, self.position))
        # return MorabarabaRules.random_play(state, self.position)
        return self.which_actions_r_guessed_2_b_the_best(state, self.position)[0]
    ...
    def evaluate(self, state, strategie):
        ...
        if strategie=='defense':
            ...
            if MorabarabaRules.is_player_stuck(state, -self.position):
                stuck=np.inf
            return len(all_my_mills) - len(all_adv_mills) + stuck - self.solve_a_menace_from(self.position, state)[2]*10   
        ...
        if strategie=='attack':
            ...
            if MorabarabaRules.is_player_stuck(state, -self.position):
                stuck=np.inf
            return len(my_pieces_on_board) - len(adv_pieces_on_board) + stuck + self.solve_a_menace_from(self.position, state)[2]*10
